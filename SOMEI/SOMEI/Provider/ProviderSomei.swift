...
class ProviderSomei {
    ...
    class func requestPerfil(id:String,email:String, password:String,completion: @escaping (Bool) -> Void){
        ...
        let loginData = loginString.data(using: String.Encoding.utf8)!
        let base64LoginString = loginData.base64EncodedString()
        let completeUrl = "https://somei-app-server.herokuapp.com/api/v1/profissional/\(id)/perfil"
                ...
                if response.statusCode == 200 {
                    ...
                    do{
                        if let json = try JSONSerialization.jsonObject(with: data, options: .mutableLeaves) as? Dictionary<String, AnyObject> {
                            print(json)
                            let ProfissionalPerfil = Profissional.byDictPerfil(dict: json, password: password)
                            ProfissionalManager.sharedInstance.receivedPerfil(perfil: ProfissionalPerfil)
...
//                            print(dctJson)
    ...
    class func sendImageToAPI(photoURL:String, id:String,email:String, password:String,onComplete: @escaping (Bool) -> Void) {
        let loginString = String(format: "%@:%@", email, password)
        let loginData = loginString.data(using: String.Encoding.utf8)!
        let base64LoginString = loginData.base64EncodedString()
        let completeUrl = "https://somei-app-server.herokuapp.com/api/v1/profissional/\(id)/porftolio?foto=\(photoURL)"
